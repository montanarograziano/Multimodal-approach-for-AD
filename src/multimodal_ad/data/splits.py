...
def assert_no_subject_leakage(
    train_df: pd.DataFrame, test_df: pd.DataFrame, *, subject_col: str = "subject_id"
) -> None:
...
def stratified_subject_folds(
    ...
    *,
    ...
    Each fold's `(train_index, test_index)` pair references `df.index`.
    No subject appears in both the train and test side of the same fold.
    """
    splitter = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=seed)
    folds: list[tuple[pd.Index, pd.Index]] = []
    for train_pos, test_pos in splitter.split(df, y=df[label_col], groups=df[subject_col]):
        train_index = df.index[train_pos]
        test_index = df.index[test_pos]
        assert_no_subject_leakage(df.loc[train_index], df.loc[test_index], subject_col=subject_col)
